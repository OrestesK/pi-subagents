...
const AcceptanceOverride = Type.Unsafe({
	anyOf: [
		{
			type: "string",
			enum: ["auto", "none", "attested", "checked", "verified", "reviewed"],
		},
...
const MaxOutputOverride = Type.Object(
	...
	},
	{
		additionalProperties: false,
		description:
			"Optional parent-visible output limits. Omitted fields use the 200 KiB and 5,000-line defaults.",
	},
);
...
const SubagentParamsSchema = Type.Object({
	workflow: Type.Optional(
		Type.String({
			...
			description:
				"Deprecated compatibility alias for named builtin workflows. Prefer prompt shortcuts or explicit tasks/chain. Requires task. Mutually exclusive with agent, tasks, chain, action, config, and chainName. Builtin workflows run foreground/fresh by default so the parent can synthesize the result before proceeding.",
		}),
	),
	agent: Type.Optional(
		Type.String({
	...
	id: Type.Optional(
		Type.String({
			description:
				"Run id or prefix for action='status', action='interrupt', action='resume', action='steer', or action='append-step'.",
		}),
	),
	runId: Type.Optional(
		Type.String({
			description:
				"Target run ID for action='interrupt', action='resume', action='steer', or action='append-step'. Defaults to the most recently active controllable run for interrupt. Prefer id for new calls.",
		}),
	),
	dir: Type.Optional(
		Type.String({
			description:
				"Async run directory for action='status', action='resume', or action='steer'.",
		}),
	),
	index: Type.Optional(
		Type.Integer({
	...
	offset: Type.Optional(
		Type.Integer({
			minimum: 0,
			description:
				"Live status pagination offset for view='fleet' or the default active-run list.",
		}),
	),
	lines: Type.Optional(
		Type.Integer({
	...
	message: Type.Optional(
		Type.String({
			description:
				"Follow-up message for action='resume' or non-terminal guidance for action='steer'. Use index to choose a child from multi-child runs.",
		}),
	),
...
const WaitParamsSchema = Type.Object({
	id: Type.Optional(
		Type.String({
			description:
				"Run id or prefix to wait for one specific run. Omit to wait across every active async run started in this session.",
		}),
	),
	all: Type.Optional(
		Type.Boolean({
