...
• PARALLEL: { tasks: [{agent,task,count?,output?,reads?,progress?,toolExtensions?,requiresCapabilities?}, ...], concurrency?: number, worktree?: true } - concurrent execution (worktree: isolate each task in a git worktree)
• Per-child tool additions: { toolExtensions: { add: ["bundle-id"] }, requiresCapabilities: ["mcp"] } - use configured bundle IDs shown by action:list; never pass raw paths or transport configuration.
...
• Add configured builtin tools per child with { toolExtensions: { add: ["bundle-id"] }, requiresCapabilities: ["mcp"] }; discover bundle IDs with action:list.
