...
export const SUBAGENT_SAFETY_GUIDANCE = `SUBAGENT EXECUTION GUIDANCE:
• ${AGENT_SELECTION_GUIDANCE}
• Omit action for execution. For a delegated multi-step/parallel workflow: exactly one top-level subagent workflow call with async:true; children launch only inside it.
...
• For durable output, return actual outputReference, outputPathMapping, or artifactPaths. Set output only when a caller-selected path is required; task filename prose does not bind runtime output.
• children.list: resume only resumable rows. {action:"resume",id,message} detaches a follow-up with the stored agent/model/tool contract. Scripts await runs.run(newKey,{resume:runId,task}); continue from the latest returned runId. Each distinct resume pass needs a new stable key; same-key reuse requires identical launch parameters.
...
export const SUBAGENT_TOOL_PROMPT_SNIPPET = "Delegate to subagents; compose multi-child work in one workflow call.";
export const SUBAGENT_TOOL_PROMPT_GUIDELINES = [
	`Subagent execution uses this discovery preflight. ${AGENT_SELECTION_GUIDANCE}`,
...
function withMandatorySafetyGuidance(description: string): string {
	const customDescription = description.split(SUBAGENT_SAFETY_GUIDANCE)
		.map((part) => part.trim())
		.filter(Boolean)
		.join("\n\n");
