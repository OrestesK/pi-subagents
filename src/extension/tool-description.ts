...
ASYNC / WAIT:
• async:true detaches background work. Do not sleep or poll just to wait. Persistent interactive parents should continue useful work. During waits, they may do independent reflection or permitted internal-state maintenance, but only when this work cannot delay required work. When no useful work, independent reflection, or permitted maintenance remains, yield. Completion notifications resume them without another user prompt. Inspect relevant completed outputs before dependent decisions or final claims. Use the wait tool only for non-yielding/run-to-completion flows or a named same-control-flow dependency; prefer foreground execution for a known immediate dependency.
