...
export function formatAsyncStartedMessage(headline: string): string {
	return [
		...
		"When a known immediate dependency requires child output, use foreground execution instead of launching it asynchronously. Use subagent({ action: \"status\", id: \"...\" }) only for a one-shot inspection of a blocked or stale run.",
	].join("\n");
}
...
export function buildAsyncRunnerSteps(id: string, params: AsyncRunnerStepBuildParams): AsyncRunnerStepBuildResult {
	...
	let flatStepIndex = 0;
	const nextFlatStep = (): { index: number; sessionFile?: string; thinkingOverride?: string } => {
...
export function executeAsyncChain(
	...
	const chainDesc = chain
		.map((s) => isDynamicParallelStep(s) ? `expand:${s.parallel.agent}` : formatChainStepLabel(s))
		.join(" -> ");
				...
				steps: [
					{
						...
						model,
						thinking: resolveEffectiveThinking(model, effectiveThinking),
						modelCandidates: buildModelCandidates(primaryModel, agentConfig.fallbackModels, availableModels, ctx.currentModelProvider, { scope: ctx.modelScope })
