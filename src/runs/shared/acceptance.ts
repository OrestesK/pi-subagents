...
function requiredEvidenceForLevel(level: Exclude<AcceptanceLevel, "auto">): AcceptanceEvidenceKind[] {
	switch (level) {
		...
		case "checked":
			return ["changed-files", "commands-run", "residual-risks"];
		case "verified":
		case "reviewed":
			return ["changed-files", "commands-run", "validation-output", "residual-risks"];
	}
}
...
}): { level: Exclude<AcceptanceLevel, "auto">; reasons: string[]; criteria: string[]; evidence: AcceptanceEvidenceKind[]; review?: AcceptanceReviewGate | false } {
...
export function normalizeAcceptanceInput(input: AcceptanceInput | undefined): AcceptanceConfig {
	...
	if (input === "none") return { level: "none", reason: "disabled by explicit none shorthand" };
