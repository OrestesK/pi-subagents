...
- Async launch, tool, skill, and README guidance treats persistent interactive completion as notification-driven and reserves `wait` for non-yielding/run-to-completion or named same-control-flow dependencies; the aggressive Reflection during waits policy remains owned by the global agent instructions.
