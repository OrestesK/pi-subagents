...
- Background completion notifications include run id, role, cwd when present, launch time, and output path when configured, and suppress stale completions older than the notification dedupe TTL. Every native completion uses `{ triggerTurn: true, deliverAs: "followUp" }`; an acknowledged intercom delivery queues a hidden metadata-only native wake-up (`display: false`) with status and retrieval pointers, so streaming parents continue without receiving the result preview twice.
